...
const SideNavbar = () => {
  ...
  useEffect(() => {
    const currentMenu = MENU_LIST.find((menu) => menu.path === location.pathname);
    if (currentMenu) {
    ...
    <div className={nav.container}>
      ...
      <div className={nav.lower}>
        {MENU_LIST.map((menu) => {
          ...
          return (
            <div
              className={clsx(nav.menu, isSelected && nav.selected)}
              key={menu.key}
              onClick={() => setSelectedMenu(menu.key)}
            >
              <Link to={menu.path}>
                <img className={nav.icon} src={isSelected ? menu.activeIcon : menu.icon} alt={menu.alt} />
              </Link>
            </div>
          );
        })}
